...
void test_detector_2(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/train.list");
    ...
    for(i = nthreads; i < m+nthreads; i += nthreads){
        ...
        for(t = 0; t < nthreads && i+t-nthreads < m; ++t){
            ...
            image **alphabet = load_alphabet();
            draw_detections(val[t], dets, nboxes, thresh, names, alphabet, l.classes);
            save_image(val[t], "predictions");
            free_detections(dets, nboxes);
            free(id);
            free_image(val[t]);
            free_image(val_resized[t]);
...
void run_detector(int argc, char **argv)
{
    ...
    if(0==strcmp(argv[2], "testmulti")) test_detector_2(datacfg, cfg, weights, filename, thresh, hier_thresh, outfile, fullscreen);
